...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    let activeCategoriesString = activeCategories.map(category => `'${category}'`).join(',');

    db.transaction((tx) => {
      tx.executeSql(
        `SELECT * FROM menuitems WHERE title LIKE ? AND category IN (${activeCategoriesString})`,
        [`%${query}%`],
        (_, result) => {
        ...
        (_, error) => {
          console.error(error);
          reject(error);
          return true; // stops passing the error to the promise
        }
      );
    });
  });
